...
introduction
use this module to operate PostGreSQL database,
execute sql or save data to database.

Usage
=====
>>> from database_manager.postgre_dbm.database_manager import DatabaseManager
...
class DatabaseManager(object):
    ...
    def save(self, table_name, record_dict, is_insert_id=False):
        ...
        try:
            if is_insert_id:
                self.cursor.execute(sql)
                success_column = int(self.cursor.rowcount)
            else:
